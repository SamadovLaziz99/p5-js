...
function sharStopFalse(event) {
    this.sharStop = event;
    return this.sharStop;
}
...
function draw() {
    ...
    if (Math.pow(radius, 2) >= (Math.pow(xCoords, 2) + Math.pow(yCoords, 2))) {
        console.log("tegdi");
        sharStop = false;
        console.log('sharStop1: ', sharStop);
        shar_1.speed = 0;
        shar_2.speed = 0;
        // console.log(Math.pow(radius, 2), Math.pow(xCoords, 2) + Math.pow(yCoords, 2));
        shar_1.vx = shar_1.vx * (-1);
        shar_1.vy = shar_1.vy * (-1);
        shar_2.vx = shar_2.vx * (-1);
        shar_2.vy = shar_2.vy * (-1);
        this.sharStopFalse(true);
        if(this.sharStopFalse) {
            shar_1.speed = 1;
            shar_2.speed = 2;
        }
    }
